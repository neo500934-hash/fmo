...
(function() {
  ...
  function initSidebarNav() {
    ...
    navLinks.forEach(function(link) {
      link.addEventListener('click', function(e) {
        ...
        const sidebarNav = this.closest('.sidebar-nav');
        ...
        // Keep scroll position stable to avoid visible jump on click.
        ...
        // Prevent pointer click focus from triggering extra scroll adjustments.
        if (e.detail > 0) {
          this.blur();
        }
      });
   ...
   * Open a submenu
   */
  function openSubmenu(item, animate = false) {
  ...
  function closeSubmenu(item) {
    ...
    // Recalculate ancestor heights after this submenu collapses.
    requestAnimationFrame(function() {
      const parentOpen = item.parentElement.closest('.has-submenu.open');
      if (parentOpen) {
        updateParentHeight(parentOpen);
      }
    });
  }
  ...
  function updateParentHeight(item) {
    ...
    while (current) {
      if (current.classList.contains('has-submenu') && current.classList.contains('open')) {
        const currentSubmenu = current.querySelector(':scope > .nav-submenu');
        if (currentSubmenu) {
          currentSubmenu.style.maxHeight = currentSubmenu.scrollHeight + 'px';
        }
      }

      current = current.parentElement ? current.parentElement.closest('.has-submenu.open') : null;
    }
  }
